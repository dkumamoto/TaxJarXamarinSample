...
namespace UnitTest_TaxJar
{
    ...
    public class Tests
    {
        ...
        public void Setup()
        {
            _taxCalc = new CalculatorTaxJar(productionFlag:true);
            _taxJarConverters = new TaxJarConverters();
        }

        [TestCase("90404-3370", 0.1025f)]
        [TestCase("05495-2086", 0.07f)]
        public async Task GetTaxRate_UsingSampleZip_ShouldGetTaxRate(string zip, float expectedRate)
        {
        ...
        [TestCase("baddata")]
        public async Task GetTaxRate_UsingInvalidZip_ShouldGetErrors(string zip)
        {
        ...
        [TestCase]
        public async Task CalcTaxOfOrder_UsingSampleOrder_ShouldGetCorrectTax()
        {
